...
def get_tsb_zone(tsb_value):
    """Determinar zona TSB y retornar información"""
    zones = [
        {'range': '+10 a +20', 'zone': 'Pico de forma', 'color': '🟢', 'action': 'Carrera o sesión de calidad máxima', 'emoji': '🏆'},
        {'range': '+3 a +10', 'zone': 'Fresco', 'color': '🟢', 'action': 'Entrenamiento normal con calidad', 'emoji': '✅'},
        {'range': '0 a -5', 'zone': 'Cargado', 'color': '🟡', 'action': 'Entrenar pero monitorear sensaciones', 'emoji': '⚡'},
        {'range': '-5 a -15', 'zone': 'Fatiga normal', 'color': '🟠', 'action': 'Reducir intensidad, mantener volumen moderado', 'emoji': '⚠️'},
        {'range': '-15 a -25', 'zone': 'Fatiga alta', 'color': '🟠', 'action': 'Solo Z1-Z2, sin sesiones de calidad', 'emoji': '⚠️'},
        {'range': '-25 a -35', 'zone': 'Zona de riesgo', 'color': '🔴', 'action': 'Descanso activo únicamente', 'emoji': '🛑'},
        {'range': 'Debajo de -35', 'zone': 'Peligro real', 'color': '🔴', 'action': 'Descanso completo obligatorio', 'emoji': '🚨'},
    ]
...
def show_alerts(wellness_last, bodycomp_last, wellness_df):
    """Mostrar cuadro de alertas con escala TSB"""
    alerts_critical = []
    alerts_warning = []
    alerts_info = []
...
def main():
    ...
    with col1:
        ...
        if not wellness_last.empty:
            ...
            # Obtener zona TSB
            tsb_zone = get_tsb_zone(tsb_now)
            
            # Mostrar métrica principal
            st.metric("TSB (Forma)", f"{tsb_now:.0f}", f"{tsb_trend} {tsb_change:+.1f}")
            
            # Mostrar zona y acción
            st.markdown(f"**{tsb_zone['zone']}** ({tsb_zone['range']})")
            st.markdown(f"_{tsb_zone['action']}_")
